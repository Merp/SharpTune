...
{
	/// <author>Steve Wadsworth <lizzardo on RomRaider forum></author>
	/// <since>
	/// 2012-12-30
	...
	/// </since>
	public sealed class JREChecker
	{
		/// <summary>Check to see if the JVM is 32-bit</summary>
		/// <returns>true if JVM is 32-bit</returns>
		public static bool Is32bit()
		{
			// determine if we're running in a 32-bit JVM
			// this may need to be extended for other JVM providers and versions
			string bitness = Runtime.GetProperty("sun.arch.data.model", "unknown");
			...
			if (bitness.Equals("unknown"))
			{
				...
				if (Runtime.GetProperty("java.vm.name").IndexOf("64") >= 0 || Runtime.GetProperty
					("sun.cpu.isalist").IndexOf("64") >= 0)
				{
					bitness = "64";
				}
			}
