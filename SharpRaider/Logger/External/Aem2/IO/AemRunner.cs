...
namespace RomRaider.Logger.External.Aem2.IO
{
	public sealed class AemRunner : Stoppable
	{
		private static readonly Org.Apache.Log4j.Logger LOGGER = Org.Apache.Log4j.Logger.GetLogger
			(typeof(RomRaider.Logger.External.Aem2.IO.AemRunner));
		...
		public void Run()
		{
			try
			{
				while (!stop)
				{
					...
					{
						dataItem.SetData(ParseString(response));
					}
				}
				connection.Close();
			}
