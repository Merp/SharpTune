...
namespace RomRaider.Logger.Ecu.Comms.IO.Protocol
{
	public interface LoggerProtocol
	{
		...
		byte[] ConstructReadAddressRequest(byte id, ICollection<EcuQuery> queries);

		byte[] ConstructReadAddressResponse(ICollection<EcuQuery> queries, PollingState pollState
			);
		...
		void ProcessReadAddressResponses(ICollection<EcuQuery> queries, byte[] response, 
			PollingState pollState);

		RomRaider.IO.Protocol.Protocol GetProtocol();
	}
}
